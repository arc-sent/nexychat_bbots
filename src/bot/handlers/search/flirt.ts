...
export const FlirtSearch = new Scenes.WizardScene<MyContext>('flirtSearch', async (ctx) => {
    ...
    try {
        ...
        if (dataMessage.age < 18) {
            await ctx.reply(
                `🔒 <b>Доступ ограничен</b>\n\n<i>Функция доступна только для пользователей, достигших 18 лет.</i>`,
                {
                    parse_mode: 'HTML',
                    reply_markup: {
                        keyboard: [
        ...
        const exists = await redis.exists(`queue:flirt:${ctx.from.id}`);

        if (exists) {
            await redis.del(`queue:flirt:${ctx.from.id}`);
        }

        await redis.del(`session:flirt:${ctx.from?.id}`);

        await redis.hset(`queue:flirt:${ctx.from.id}`, user)
    ...
    const endChat = await redis.hget(`session:flirt:${ctx.from?.id}`, 'end');
    ...
    if (targetendChat == 1) {
        ...
        if ('data' in ctx.callbackQuery) {
            ...
            if (data.startsWith('rate_')) {
                ...
                const messageId = await redis.hget(`session:flirt:${ctx.from.id}`, 'ratingId');

                console.log(`session:flirt:${ctx.from.id} messageId`, Number(messageId));
    ...
    if (!partnerId) {
        ...
        if ('text' in ctx.message) {
            ...
            switch (message) {
                case '⛔ Завершить чат':
                    const start = await redis.hget(`session:flirt:${ctx.from?.id}`, 'start');
                    ...
                    if (durationMin >= 1) {
                        ...
                        await redis.hset(`session:flirt:${ctx.from?.id}`, 'end', '1');
                        await redis.hset(`session:flirt:${ctx.from?.id}`, 'ratingId', ctx.session.sendMessageTalk);

                        await redis.hset(`session:flirt:${partnerId}`, 'end', '1');
                        await redis.hset(`session:flirt:${partnerId}`, 'ratingId', messageId);

                        const endCtxId = await redis.hget(`session:flirt:${ctx.from?.id}`, 'end');

                        console.log(`session:flirt:${ctx.from?.id}`, endCtxId);
                    ...
                    await redis.del(`session:flirt:${ctx.from?.id}`);
                    await redis.del(`session:flirt:${partnerId}`);
